...
Stage00 + Stage10 (COPY-ONLY) pipeline.

- Robust against cfg shape mismatch (env-first resolution)
- COPY first, EDIT later (edit hook will be added next)
"""
...
def run_multistage(dbx: DropboxIO, cfg, run_id: str | None = None) -> int:
    ...
    store.audit_event(run_id=rid, stage="--", event="run_start",
                      message="monthly pipeline start (stage00+10 copy-only)")
    ...
    try:
        ...
        store.audit_event(run_id=rid, stage="00", event="list",
                          src_path=s00_in, count=len(files00))
        cnt = 0
        for f in files00:
            if cnt >= max_files:
                store.audit_event(run_id=rid, stage="00", event="stop",
                                  message="max_files_per_run", limit=max_files)
                break
            src = f.path_display or f.path_lower or ""
            base = src.split("/")[-1]
            key = stable_key(src)
            if store.is_processed(key):
                store.audit_event(run_id=rid, stage="00", event="skip",
                                  src_path=src, filename=base,
            ...
            store.audit_event(run_id=rid, stage="00", event="write",
                              src_path=src, dst_path=outp, filename=base, size=len(b))

            dbx.move(src, donep)
            store.audit_event(run_id=rid, stage="00", event="move",
                              src_path=src, dst_path=donep, filename=base)

            dbx.write_file_bytes(nextp, b, overwrite=True)
            store.audit_event(run_id=rid, stage="10", event="write",
                              src_path=donep, dst_path=nextp, filename=base,
        ...
        store.audit_event(run_id=rid, stage="00", event="error",
                          message=str(e), traceback=tb.format_exc())
    ...
    try:
        ...
        store.audit_event(run_id=rid, stage="10", event="list",
                          src_path=s10_in, count=len(files10))
        cnt = 0
        for f in files10:
            if cnt >= max_files:
                store.audit_event(run_id=rid, stage="10", event="stop",
                                  message="max_files_per_run", limit=max_files)
                break
            src = f.path_display or f.path_lower or ""
            base = src.split("/")[-1]
            ...
            if store.is_processed(key):
                store.audit_event(run_id=rid, stage="10", event="skip",
                                  src_path=src, filename=base,
                                  message="already_processed")
                continue
            ...
            dbx.move(src, donep)
            store.audit_event(run_id=rid, stage="10", event="move",
                              src_path=src, dst_path=donep, filename=base)

            dbx.write_file_bytes(nextp, b, overwrite=True)
            store.audit_event(run_id=rid, stage="20", event="write",
                              src_path=donep, dst_path=nextp, filename=base,
        ...
        store.audit_event(run_id=rid, stage="10", event="error",
                          message=str(e), traceback=tb.format_exc())

    # ---- persist ----
    try:
        store.save(dbx)
        store.audit_event(run_id=rid, stage="--", event="write_state",
                          filename=state_path, message="state saved")
    except Exception as e:
        store.audit_event(run_id=rid, stage="--", event="error",
                          message=f"state_save_failed: {e}", traceback=tb.format_exc())

    store.audit_event(run_id=rid, stage="--", event="run_end",
